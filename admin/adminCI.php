...
$sql = "SELECT id, fullName, ntnNumber, cnicNo, emailAddress, phoneNo, dateofBirth, graduationYear, education, discipline, industry, preferredCity, registrationTimestamp, address FROM currentinternees WHERE internship_status = 'active'";
...
<body>
              ...
              <table id="dataTable" class=" w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  ...
                  while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<button class='px-2 border text-green-500  py-1 mr-2' onclick='markCompleted(" . $row['id'] . ")'>Completed Internship</button>";
                    echo "<button class='px-2 border text-blue-500 py-1' onclick='markQuitted(" . $row['id'] . ")'>Quitted Internship</button>";
  ...
  <script>
    function markCompleted(internId) {
      // Send an asynchronous request to a PHP script to handle the completion logic
      ...
      xmlhttp.open("GET", "mark_completed.php?id=" + internId, true);
      xmlhttp.send();
    }
    function markQuitted(id) {
      $.ajax({
        type: 'POST',
        url: 'markQuitted.php', // Adjust the path to your PHP script
        data: { id: id },
        success: function (response) {
          alert(response);
          // Optionally, you can update the table without a page refresh
          $('#row_' + id).remove();
        },
        error: function () {
