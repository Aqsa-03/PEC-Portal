...
$sql = "SELECT id, ntnNumber, fullName, emailAddress, phoneNo, discipline FROM internees";
...
<body>
    ...
    <div class="flex flex-col flex-1 w-full">
      ...
      <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">Engineer Contact Details</h2>
          <!-- CTA -->
              ...
              <table class="w-full whitespace-no-wrap" id="dataTable">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  ...
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr id='row_" . $row['id'] . "' class='text-gray-700 dark:text-gray-400'>";
                    echo "<td class='px-4 py-3 text-sm'>" . $row['id'] . "</td>";
                    echo "<td class='px-4 py-3'>" . $row['ntnNumber'] . "</td>";
                    echo "<td class='px-4 py-3 text-sm'>" . $row['fullName'] . "</td>";
                    echo "<td class='px-4 py-3 text-xs'><span class='px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100'>" . $row['emailAddress'] . "</span></td>";
                    ...
                    // "Contacted" button
                    echo "<button class='px-2 border text-green-500 py-1 mr-2' onclick='markContacted(" . $row['id'] . ", true)'>Contacted</button>";

                    // "Not Contacted" button
                    echo "<button class='px-2 border text-red-500 py-1' onclick='markContacted(" . $row['id'] . ", false)'>Not Contacted</button>";
  ...
  <script>
    function markContacted(id, contacted) {
      // Send an AJAX request to the server
      var xhr = new XMLHttpRequest();
      xhr.open('POST', 'mark_contacted.php', true);
      xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');

      // Send the ID and status (contacted or not contacted) to the server
      xhr.send('id=' + id + '&contacted=' + contacted);

      // Change the background color of the row based on the status
      var row = document.getElementById('row_' + id);
      row.style.backgroundColor = contacted ? '#8FDB94' : ''; // Green for contacted, remove color for not contacted
    }
  </script>
